...
static int my_exec_tierce(sudo_t *sudo_struct, char **argv, int *i)
{
    ...
    if (sudo_struct->g != 0) {
        setregid(my_getgid(argv[sudo_struct->g + 1]),
        my_getgid(argv[sudo_struct->g + 1]));
    } else {
        setregid(my_getgid_bis(sudo_struct->user),
        my_getgid_bis(sudo_struct->user));
    }
    setgroups(0, NULL);
...
static int my_exec_bis(sudo_t *sudo_struct, char **argv, char **env)
{
    ...
    if (sudo_struct->u != 0) {
        if (my_getuid(argv[sudo_struct->u + 1]) == 84 ||
            setreuid(my_getuid(argv[sudo_struct->u + 1]),
            my_getuid(argv[sudo_struct->u + 1])) == -1)
            return 84;
    } else {
